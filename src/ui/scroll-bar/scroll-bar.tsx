...
const ScrollBar = forwardRef<HTMLDivElement | null, ScrollBarProps & AreaHTMLAttributes<HTMLDivElement>>(
    function ScrollBarRef(
        {children, initScroll = 0, onToScroll, className, os = 'X', valueScrool = 16, onTransitionEnd, ...props},
        ref
    ) {
        const {refBar, refWrapp, scroll, handalDrag, handalDragEnd, handalDragStart, hendalScroll, hendalTouch} =
        ...
        return (
            <div
                ref={refBar}
                onMouseMove={handalDrag}
                onMouseUp={handalDragEnd}
                onMouseDown={handalDragStart}
                onWheel={hendalScroll}
                onTouchMove={hendalTouch}
